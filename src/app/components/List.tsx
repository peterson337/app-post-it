...
export const List = ({
  ...
  setListaDeCompra: React.Dispatch<React.SetStateAction<TarefasDeCompra[]>>;
  precoTotal: number;
  setPrecoTotal: Dispatch<SetStateAction<number>>;
}) => {
      ...
      ListaDeCompra.find((item) => {
        if (item.id === id) {
          ...
          if (precoTotal !== null && ArmazenarValueNUmber !== null) {
            ...
            const atualizarTarefaFavorita = ListaDeCompra.map(
              (val: TarefasDeCompra) =>
                val.id === id
                  ? {
                      ...val,
                      tarefa: atualizarTarefaDeCompra,
                      preco: ArmazenarValueNUmber,
                      precoTotal: resultadoFinal,
                    }
                  : val
            );

            setListaDeCompra(
              atualizarTarefaFavorita as unknown as TarefasDeCompra[]
            );
              ...
              <div className="grid  xl:grid-cols-3 lg:grid-cols-2 md:grid-rows-4">
                {ListaDeCompra.map((item: TarefasDeCompra, index: number) => {
                  ...
                  flex  flex-col justify-between ${
                    tarefaSalva ? "bg-green-500" : "bg-red-500"
                  }
                 border-0 rounded-lg w-[288px] gap-3 md:w-80  `}
                        >
                          {tarefaEmEdicaoId != item.id ? (
                            <>
                              <div className=" w-[270px] md:w-72 overflow-auto scrollbar-thin p-1 h-14 ">
                                <p
                                  className={`${
                                    tarefaSalva ? "line-through" : ""
                                  } `}
                                >
                                  {item.tarefa}
                                </p>
                            ...
                            <>
                              <textarea
                                className="text-black  w-60 md:w-72 p-3 rounded-xl pl-3 
                    outline-none resize-none "
                                value={atualizarTarefaDeCompra}
                                onChange={(e) =>
                                  setAtualizarTarefaDeCompra(e.target.value)
                                }
                              />
                          ...
                          <div className="flex  flex-row justify-between	 ">
                            <button
                              onClick={
                                tarefaEmEdicaoId === null
                                  ? () => editInputTeactAndInputNumber(item.id)
                                  : atualizarTarefaDeCompra.length === 0 ||
                                    ArmazenarValueNUmber === null
                                  ? () => setTarefaEmEdicaoId(null)
                                  : tarefaEmEdicaoId === item.id
                                  ? () => atualizarTarefaFavorita(item.id)
                                  : () => setTarefaEmEdicaoId(null)
                              }
                            >
                              {tarefaEmEdicaoId === item.id ? (
                            ...
                            <button
                              onClick={() => MacarTarefaComoConcluida(item.id)}
                            >
                              <FaCheck></FaCheck>
                            </button>
                          </div>
