  ...
  const handlerSort = () => {
    if (
      ...
      localStorage.setItem(
        "tarefasFavoritas",
        JSON.stringify(ListaDeCompraClone)
      );
    }
  };

  const apagarTodasAsTarefasConcluidas = () => {
    const temTarefaconcluida = tarefasFavoritas.some(
    ...
    const deletarTarefa = tarefasFavoritas.filter(
     ...
     w-full break-words h-36 overflow-auto font-bangers`}
                    ...
                    <CardActions className=" flex flex-row justify-between">
                      <button
                        ...
                        onClick={() => excluirTarefasFavorita(val.id)}
                      >
                        <FaTrash />{" "}
                      </button>
                      <button
                        className="hover:bg-gray-300 p-3 hover:rounded-full text-green-500"
                        onClick={() => MacarTarefavoritaComoConcluida(val.id)}
                      >
                        <FaCheck />{" "}
                      </button>
                      <button
                        className="hover:bg-gray-300 p-3 hover:rounded-full "
                        onClick={() => desfavoritarTarefa(val.id)}
                      >
                        <BsBookmarkStar />{" "}
                      </button>
                      {/* <button
